...
def construct_data_from_graph_gvp(protein_node_xyz, protein_seq, protein_node_s, 
    ...
    n_node = protein_node_xyz.shape[0]     # 这两行代码计算了蛋白质节点的数量和配体节点的数量
    n_compound_node = coords.shape[0]
    # centroid instead of com. 
    com = coords.mean(axis=0)              # 计算配体的质心坐标。
    keepNode = get_keepNode(com, protein_node_xyz.numpy(), n_node, pocket_radius, use_whole_protein, 
                             use_compound_com_as_pocket, add_noise_to_com, chosen_pocket_com)   # 使用get_keepNode函数来选择距离质心在一定半径范围内的蛋白质节点，并将这些节点的索引存储在keepNode变量中。

    if keepNode.sum() < 5:   # 检查所选节点的数量是否小于5。如果是，它将前100个节点标记为True，以确保有足够的节点来构建图。
        # if only include less than 5 residues, simply add first 100 residues.
        keepNode[:100] = True
    
    # 基于所选的节点索引来选择输入中与之对应的蛋白质节点的坐标（input_node_xyz），以及对应的边索引和特征。
    ...
    data = HeteroData()        # 创建一个HeteroData对象

    # only if your ligand is real this y_contact is meaningful.
    # 计算输入蛋白质节点和配体节点之间的距离矩阵（dis_map），并使用一个距离阈值（contactCutoff）来将它们分成接触和非接触两类，将结果存储在y_contact变量中
    ...
    if includeDisMap:
        ...
        dis_map[dis_map>interactionThresholdDistance] = interactionThresholdDistance   # 将距离矩阵中所有大于 interactionThresholdDistance 的距离值设置为 interactionThresholdDistance
        data.dis_map = torch.tensor(dis_map, dtype=torch.float).flatten()    # flatten() 将矩阵展平成一维向量。

    # additional information. keep records.
    data.node_xyz = input_node_xyz           # 存储选定的蛋白质节点的坐标
    data.coords = torch.tensor(coords, dtype=torch.float)     # 存储小分子的坐标
    data.y = torch.tensor(y_contact, dtype=torch.float).flatten()   # 存储蛋白质节点和小分子节点之间的接触信息
    data.seq = protein_seq[keepNode]         # 存储选定的蛋白质节点的氨基酸序列
    data['protein'].node_s = protein_node_s[keepNode] # [num_protein_nodes, num_protein_feautre]  # 存储了选定的蛋白质节点的节点特征
    data['protein'].node_v = protein_node_v[keepNode]       # 存储了选定的蛋白质节点的节点特征
    data['protein', 'p2p', 'protein'].edge_index = input_edge_idx   # 存储了选定的蛋白质节点之间的边的索引
    data['protein', 'p2p', 'protein'].edge_s = input_protein_edge_s  # 存储了选定的蛋白质边特征
    data['protein', 'p2p', 'protein'].edge_v = input_protein_edge_v  # 存储了选定的蛋白质边特征
    ...
    elif compoundMode == 1:        # compoundMode=1执行此句
